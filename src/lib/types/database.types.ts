...
/**
 * Tipo do schema usado pelo cliente Supabase tipado (generic `Database`).
 *
 * IMPORTANTE: o `@supabase/supabase-js` exige que o schema satisfaça
 * `GenericSchema`, que inclui `Tables`, `Views` e `Functions` — e cada tabela
 * precisa de `Row`, `Insert`, `Update` e `Relationships`. Se qualquer uma
 * dessas chaves faltar, o parser de `select()` não consegue resolver as
 * colunas e o resultado das queries com seleção parcial (ex.: `select("nome")`)
 * é inferido como `never`, quebrando o build com erros do tipo
...
export interface Database {
  public: {
    Tables: {
      ...
      anamnesis: TableDef<Anamnesis>;
      anthropometric_assessments: TableDef<AnthropometricAssessment>;
      foods: TableDef<Food>;
      meal_plans: TableDef<MealPlan>;
      meals: TableDef<Meal>;
      meal_items: TableDef<MealItem>;
    };
